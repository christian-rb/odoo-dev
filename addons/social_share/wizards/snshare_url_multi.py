...
class ShareWizard(models.TransientModel):
    _name = 'snshare.url.share.multi'
    _description = 'Email Social Share Campaign Wizard'


    share_campaign_id = fields.Many2one('snshare.campaign', string='Social Share Campaign', ondelete='cascade', required=True)
    ...
    def action_send(self):
        ...
        # TODO test this
        composer.write({
            'body': composer.body.replace(Markup('<div id="message"></div>'), Markup(f'<div id="message">{self.message}</div>')),
