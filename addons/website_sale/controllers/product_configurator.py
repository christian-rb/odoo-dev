...
class WebsiteSaleProductConfiguratorController(SaleProductConfiguratorController, WebsiteSale):
    ...
    @route(
        route='/website_sale_product_configurator/show_dialog',
        type='json',
        auth='public',
        website=True,
    )
    def website_sale_product_configurator_show_dialog(
        ...
        has_optional_products = bool(product_template.optional_product_ids.filtered(
            lambda op: self._show_product(op, combination)
        ))
    ...
    @route(
        route='/website_sale_product_configurator/get_values',
        type='json',
        auth='public',
        website=True,
    )
    ...
    @route(
        route='/website_sale_product_configurator/create_product',
        type='json',
        auth='public',
        methods=['POST'],
        website=True,
    ...
    @route(
        route='/website_sale_product_configurator/update_combination',
        type='json',
        auth='public',
        methods=['POST'],
        website=True,
    ...
    @route(
        route='/website_sale_product_configurator/get_optional_products',
        type='json',
        auth='public',
        website=True,
    )
    ...
    @route(
        '/website_sale_product_configurator/update_cart',
        type='json',
        auth='public',
        methods=['POST'],
        website=True,
    ...
    def website_sale_product_configurator_update_cart(
        ...
        # The main product could have a parent if there's a circularity in the optional products,
        ...
        if optional_products and values['line_id']:
            ...
            for option in optional_products:
                ...
                parent_ids[option['product_template_id']] = option['parent_product_template_id']

        for line in request.env['sale.order.line'].browse(line_ids.values()):
            parent_product_template_id = parent_ids.get(line.product_template_id.id)
            if parent_product_template_id:
                linked_line_id = line_ids[parent_product_template_id]
                if linked_line_id != line.linked_line_id.id:
                    line.sudo().write({'linked_line_id': linked_line_id})

        values['notification_info'] = self._get_cart_notification_information(
            order, line_ids.values()
        )
    ...
    def _get_basic_product_information(
        ...
        """ Override to append website data. """
        basic_product_information = super()._get_basic_product_information(
            product_or_template, pricelist, combination, currency=currency, date=date, **kwargs
        )
    ...
    def _get_ptav_price_extra(self, ptav, currency, so_date, product_or_template):
        price_extra = super()._get_ptav_price_extra(ptav, currency, so_date, product_or_template)
        if request.is_frontend:
    ...
    def _show_product(self, product_template, parent_combination):
        if request.is_frontend:
            return product_template._is_add_to_cart_possible(parent_combination)
        return super()._show_product(product_template, parent_combination)
