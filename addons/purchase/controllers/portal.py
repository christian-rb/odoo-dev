...
class CustomerPortal(portal.CustomerPortal):
    ...
    def _session_is_verified(self, data_owner_partner, session=None):
        """Verifies that logged_in_partner is in possession of a verified session to access data belonging to
        data_owner_partner.
        ...
        if data_owner_partner.id == session.get('purchase-vendor-verified-partner-id') and \
    ...
    def _session_check(self, data_owner_partner):
        ...
        """
        if not self._session_is_verified(data_owner_partner):
    ...
    def _purchase_order_get_page_view_values(self, order, access_token, **kwargs):
        ...
        values = {
            ...
            'verified_session': self._session_is_verified(order.partner_id.commercial_partner_id),
        }
    ...
    def _process_uploaded_bill(self, order_sudo, bank_account_id, uploaded_pdf):
        ...
        :param uploaded_pdf: the uploaded PDF.
        """
        self._session_check(order_sudo.partner_id.commercial_partner_id)
        ...
        if order_sudo.invoice_ids.filtered(lambda invoice: invoice.state != 'cancel'):
            raise ValidationError(_('Delete the existing bill first.'))

        # with_user() for correct create_uid, needed to know if we can cancel it ourselves from the portal.
        invoice = order_sudo.with_user(request.env.user).sudo()._create_invoices()
        invoice.partner_bank_id = bank_account_sudo
    ...
    def portal_my_purchase_order(self, order_id=None, access_token=None, bill_pdf=None, bank_account=None, expiration_ts=None, signature=None, **kw):
        ...
        if order_sudo._verify_session(request.env.user.partner_id, expiration_ts, signature):
    ...
    def portal_verify_email(self, order_id, **kw):
        ...
        if po_sudo.state not in ('purchase', 'done'):
            raise ValidationError(_('Verification can only be done on confirmed purchase orders.'))
    ...
    def portal_vendor_bill_cancel(self, move_id, purchase_order_id, **kw):
        ...
        """
        try:
            po_sudo = self._document_check_access('purchase.order', int(purchase_order_id))
            self._session_check(po_sudo.partner_id.commercial_partner_id)
        except (AccessError, MissingError):
            return request.redirect('/my')
    ...
    def _process_added_bank_account(self, **kw):
        ...
        values = {
            field: kw[field]
            for field in ResPartnerBankSudo._get_regular_frontend_bank_account_fields(country_code)
            if field in kw
        }
        values['partner_id'] = request.env.user.partner_id.commercial_partner_id.id
    ...
    def portal_get_banks(self, bic, **kw):
        ...
        """
        self._session_check(request.env.user.partner_id.commercial_partner_id)
