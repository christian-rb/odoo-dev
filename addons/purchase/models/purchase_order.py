...
class PurchaseOrder(models.Model):
    ...
    def _get_invoiced(self):
        ...
        for order in self:
            ...
            if any(
                ...
                and order.order_line.move_line_ids.filtered(lambda aml: not aml.is_downpayment)
            ):
                order.invoice_status = 'invoiced'
            else:
                order.invoice_status = 'no'

    @api.depends('order_line.move_line_ids.move_id')
    def _compute_invoice(self):
        for order in self:
            invoices = order.mapped('order_line.move_line_ids.move_id')
    ...
    def _prepare_account_move(self):
        ...
        invoice_vals = super()._prepare_account_move()
        invoice_vals.update({
