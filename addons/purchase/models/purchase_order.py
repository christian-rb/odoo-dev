...
class PurchaseOrder(models.Model):
    ...
    def _show_upload_bill_button(self, for_partner):
        """The upload bill button should only show if the PO is confirmed. To avoid "no invoicable line" errors, we also
        hide the button when there's nothing to invoice, except when the invoices can be manually deleted by the user in
        the portal."""
        self.ensure_one()

        return self.state in ['purchase', 'done'] and \
            (self.invoice_status == 'to invoice' or self.invoice_ids.filtered(lambda i: i._can_be_cancelled_by_vendor_partner(for_partner)))

    def _get_session_verification_signature(self, expiration_ts):
    ...
    def _get_session_verification_url(self):
        """Creates a session verification URL for this PO that can be used in a verification email."""
        self.ensure_one()
        expiration_ts = int(time.time()) + VERIFICATION_URL_DURATION_S
        signature = self._get_session_verification_signature(expiration_ts)
        params = {
    ...
    def _verify_session(self, logged_in_partner, expiration_ts, signature):
        """ Verifies that this verification request:
        ...
        return consteq(signature, self._get_session_verification_signature(expiration_ts))
