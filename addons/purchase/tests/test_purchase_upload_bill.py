...
from odoo.addons.purchase.controllers.portal import CustomerPortal, VERIFIED_SESSION_DURATION_S
...
class TestPurchaseUploadBill(HttpCaseWithUserPortal, MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.someone_elses_bank_account = cls.bank_account.copy(
    ...
    def _reloaded_session_is_verified(self, partner, session):
        ...
        verified = self.controller._session_is_verified(partner.commercial_partner_id, session=session)
    ...
    def _create_verified_session(self, po, session, add_access_token=False, error_if_not_verified=True):
        ...
        url = self.po._get_session_verification_url()
        if add_access_token:
    ...
    def _upload_bill(self, po=None, add_access_token=False):
        ...
        files = {"bill_pdf": StringIO("Bill PDF content.")}
    ...
    def _get_session_verification_params(self):
        parsed = urlparse(self.po._get_session_verification_url())
    ...
    def test_purchase_order_session_verification_url(self):
        ...
        self.assertTrue(self.po._verify_session(self.po.partner_id, **params), "Signature link should be valid.")
        self.assertFalse(
            self.po._verify_session(self.other_partner, **params), "Signature link should be invalid for this partner."
        )
        self.assertFalse(
            self.our_other_po._verify_session(self.po.partner_id, **params),
    ...
    def test_purchase_order_session_verification_url_expiration(self):
        ...
        with freeze_time() as freeze:
            ...
            self.assertFalse(
                self.po._verify_session(self.po.partner_id, **params), "Signature link should have expired."
            )
    ...
    def _upload_bill_flow(self, user, password):
        ...
        response = self._upload_bill()
        self.assertEqual(response.status_code, 200, "Response should have succeeded.")
        self.assertTrue(self.po.invoice_ids, "A vendor bill should have been created.")
        self.assertEqual(
        ...
        with mute_logger("odoo.http"):
            response = self._upload_bill()
        self.assertEqual(response.status_code, 400, "Uploading another bill should raise a ValidationError.")
        self.assertIn("Delete the existing bill first.", response.text, "A ValidationError should have raised.")
    ...
    def _upload_bill_view_confidential_data_flow(self, user, password):
        ...
        self.assertEqual(
            response.text.count(self.bank_account._get_masked_number()),
            0,
            "Our bank account should not be in the DOM (no verified session, and no existing bill).",
        )
        self.assertEqual(
            response.text.count(self.someone_elses_bank_account._get_masked_number()),
            0,
        ...
        self.assertEqual(
            response.text.count(self.bank_account._get_masked_number()),
            1,
            "Our bank account should be in the DOM once, in the payment card.",
        )
        ...
        self.assertEqual(
            response.text.count(self.bank_account._get_masked_number()),
            1,
            "The used bank account on the bill should still be shown.",
        )
    ...
    def test_portal_my_purchase_order_wrong_user(self):
        ...
        def test_with_user(user, password):
            ...
            response = self.url_open(self.po._get_session_verification_url())
            self.assertEqual(response.status_code, 200, "Loading the page should have succeeded.")
    ...
    def _send_verification_mail_flow(self, user, password):
        ...
        response = self.url_open(f"/my/purchase/{self.po.id}")
        self.assertTrue(
            "you must verify your email address" in response.text, "The email verification modal should be present."
        )

        url = f"/my/purchase/{self.po.id}/verify"
        with mute_logger("odoo.http"):
            self.assertIsNone(verify(url), "A verification email should not have been sent for an RFQ.")
        self.po.button_confirm()
    ...
    def _get_bank_account_flow(self, user, password):
        ...
        response = self.url_open("/my/bank_account")
        self.assertTrue(
            self.bank_account._get_masked_number() in response.text, "Our own bank account should be visible."
        )
    ...
    def _create_bank_account_request(self, omit_required_fields=False, extra_data=None):
        ...
        if not omit_required_fields:
            ...
            files.update(
                {
                    field.name: StringIO(f"{field.name} content.")
                    for field in frontend_fields
    ...
    def _bank_account_creation_flow(self, user, password):
        ...
        self.assertTrue(new_bank_account._get_masked_number() in response.text, "Bank account should be displayed.")
    ...
    def _bank_account_archive_flow(self, user, password):
        ...
        self.assertTrue(response.url.endswith("/my/bank_account"), "Should have redirected back to /my/bank_account.")
        self.assertTrue(
            self.bank_account._get_masked_number() not in response.text, "Bank account should no longer be visible."
        )
