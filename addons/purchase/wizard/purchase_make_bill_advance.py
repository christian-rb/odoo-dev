...
class PurchaseAdvancePaymentWizard(models.TransientModel):
    ...
    advance_payment_method = fields.Selection(
        string="Create Bill",
        help="A standard vendor bill is created with all the order lines ready,"
    ...
    amount_to_invoice = fields.Monetary(string="Amount to be billed", help="The amount to be billed = Purchase Order Total - Confirmed Down Payments.")
    ...
    def view_draft_invoices(self):
        res = super().view_draft_invoices().update({'name': _('Draft Bills')})
        res.domain.append(('line_ids.purchase_order_id.id', '=', self.order_ids.id))
        return res
