...
class PortalWizardUser(models.TransientModel):
    ...
    def _get_similar_user_domain(self, email):
        ...
        than the current partner depending on its linked website
        characteristics.
        :param string email: the email of the current partner
        """
        similar_user_domain = super(PortalWizardUser, self)._get_similar_user_domain(email)
        if self.partner_id.website_id:
            # If the partner is linked to a website, only keep the users that
            # are linked to this website.
            similar_user_domain.append(('website_id.id', '=', self.partner_id.website_id.id))
        else:
    ...
    def _get_same_email_error_message(self, user_name):
        ...
        :param user_name: The user that has the same email and is linked to the same
        website than the current partner
        """
