...
class TestWebsiteResetPassword(HttpCase):
    ...
    def test_02_multi_user_login(self):
        ...
        self.env["res.users"].create(
            {"website_id": False, "login": "bobo@mail.com", "name": "AAA", "password": "bobo@mail.com", "groups_id": [Command.link(portal_group.id), Command.unlink(internal_group.id)]}
        )
        user2 = self.env["res.users"].create(
            {"website_id": website.id, "login": "bobo@mail.com", "name": "ZZZ", "password": "bobo@mail.com", "groups_id": [Command.link(portal_group.id), Command.unlink(internal_group.id)]}
        )
