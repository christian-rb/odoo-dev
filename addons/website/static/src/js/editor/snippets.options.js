...
options.registry.Carousel = options.Class.extend({
    ...
    start: function () {
        ...
        // Activate the active slide if the content changed.
        this.$target.on("content_changed.carousel_option", () => {
    ...
    async notify(name, data) {
        ...
        if (name === 'add_slide') {
            ...
            await this._slide(data.direction);
            data.onSuccess();
        }
    },
    ...
    _slide(direction) {
        ...
        this.$target.one("slid.bs.carousel", () => {
            ...
            setTimeout(() => {
                ...
                this.$target.trigger("active_slide_targeted"); // TODO remove in master: kept for stable.
            }, 0.2 * _slideDuration);
        });

        return new Promise(resolve => {
            this.$target.one("active_slide_targeted", () => {
...
options.registry.CarouselItem = options.Class.extend({
     ...
     * @see this.selectClass for parameters
     */
    async slide(previewMode, widgetValue, params) {
