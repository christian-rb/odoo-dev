...
class PosOrderLine(models.Model):
    ...
    def _get_stock_moves_to_consider(self, stock_moves, product):
        bom = product.env['mrp.bom']._bom_find(product, company_id=stock_moves.company_id.id, bom_type='phantom')[product]  # no change
        if not bom:
...
class PosOrder(models.Model):
    ...
    def _get_pos_anglo_saxon_price_unit(self, product, partner_id, quantity):
        bom = product.env['mrp.bom']._bom_find(product, company_id=self.mapped('picking_ids.move_line_ids').company_id.id, bom_type='phantom')[product]  # no change
        if not bom:
