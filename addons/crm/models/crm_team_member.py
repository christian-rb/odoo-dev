...
class TeamMember(models.Model):
    ...
    lead_day_count = fields.Integer(
        ...
        help='Lead assigned to this member this last day')
    ...
    def _get_lead_from_date(self, date_from):
        return {
            (user.id, team.id): count for user, team, count in self.env['crm.lead'].with_context(active_test=False)._read_group(
                [
                    ('date_open', '>=', date_from),
