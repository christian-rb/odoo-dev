...
class RazorpayController(http.Controller):
    ...
    def razorpay_callback(self, **data):
        ...
        if code:
            ...
            request_url = url_join(razorpay_provider._get_razorpay_oauth_url(), '/iap/razorpay_proxy/v1/get_access_token')
            try:
                params = {
                    'dbuuid': dbuuid,
                    'code': data.get('code'),
    ...
    def _get_payment_provider_web_url(self, razorpay_provider, is_successful=True):
        ...
        :rtype: str
        """
        view_ref = is_successful and 'payment.action_payment_provider' or 'payment_razorpay_auth.action_payment_provider_onboarding_using_key'
