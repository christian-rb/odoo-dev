...
class PaymentProvider(models.Model):
    ...
    def _onchange_state(self):
        if self.state == 'test' or self.state == 'disabled' or self.state == 'enabled':
            self._razorpay_provider_vals()
    ...
    def _get_razorpay_oauth_url(self):
        ...
        """
        self.ensure_one()
        OAUTH_TEST_URL = const.OAUTH_TEST_URL if self.state == 'test' else const.OAUTH_URL
    ...
    def action_check_account_activate(self):
        self.ensure_one()
        if self.razorpay_account_id:
            ...
            raise UserError(_('Your Account is not activated.'))
    ...
    def action_razorpay_create_and_update_webhook(self):
        ...
        if response.get('id'):
            ...
            if self.razorpay_webhook_id and response.get('error', {}).get('code') == 400:
