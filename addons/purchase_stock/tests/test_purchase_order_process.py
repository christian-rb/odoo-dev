...
class TestPurchaseOrderProcess(PurchaseTestCommon):
    ...
    def test_02_vendor_delay_report_partially_cancelled_purchase_order(self):
        ...
        delay_reports = self.env['vendor.delay.report'].read_group(fields=["__count", "on_time_rate:sum", "qty_total:sum"], domain=[('partner_id', "=", partner.id)], groupby=["product_id"])
        self.assertEqual([rec['on_time_rate'] for rec in delay_reports], [0.0, 0.0])
        ...
        purchase_order.picking_ids.move_ids.flush_recordset()
        delay_reports = self.env['vendor.delay.report'].read_group(fields=["__count", "on_time_rate:sum"], domain=[('partner_id', "=", partner.id)], groupby=["product_id"])
        self.assertEqual([rec['on_time_rate'] for rec in delay_reports], [100.0])
