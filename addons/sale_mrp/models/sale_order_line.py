...
class SaleOrderLine(models.Model):
    ...
    def _compute_qty_to_deliver(self):
        ...
        kit_boms = self.env['mrp.bom']._bom_find(self.filtered(lambda l: l.state in ['draft', 'sent'] and l.product_id), bom_type='phantom')
        for line in self:
            ...
            if line.state == 'sale':
                ...
                boms = kit_boms[line.product_id].filtered(lambda b: not b.company or b.company == line.company_id)[:1]
    ...
    def _compute_qty_delivered(self):
        ...
        kit_boms = self.env['mrp.bom']._boms_find(order_lines, bom_type='phantom')
        for order_line in order_lines:
            ...
            if not boms and dropship:
                ...
                boms = kit_boms[order_line.product_id].filtered(lambda b: not b.company or b.company == order_line.company_id)[:1]
    ...
    def _get_qty_procurement(self, previous_product_uom_qty=False):
        ...
        bom = self.env['mrp.bom']._bom_find(self.product_id, bom_type='phantom')[self.product_id]  # no change
        if bom:
