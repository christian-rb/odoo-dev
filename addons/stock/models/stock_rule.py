...
class StockRule(models.Model):
    ...
    def _get_rule(self, product_id, location_id, values):
        ...
        while (not rule) and location_id:
            rule = rules._search_rule(route_ids, product_packaging_id, product_id, warehouse_id, self._get_rule_domain('=', location_id, values))
            location_id = location_id.location_id
        return rule

    @api.model
    def _get_rule_domain(self, operator, location, values):
        domain = ['&', ('location_dest_id', operator, location.id), ('action', '!=', 'push')]
...
class ProcurementGroup(models.Model):
    ...
    def run(self, procurements, raise_user_error=True):
        ...
        all_rules_domain = [('location_dest_id', 'parent_of', list(location_ids)), ('action', '!=', 'push')]
        if company_ids:
            all_rules_domain = expression.AND([all_rules_domain, ['|', ('company_id', '=', False), ('company_id', 'child_of', list(company_ids))]])
