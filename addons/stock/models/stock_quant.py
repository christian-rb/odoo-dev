    ...
    def check_quantity(self):
        ...
        for product, _location, lot, qty in groups:
            if float_compare(abs(qty), 1, precision_rounding=product.uom_id.rounding) > 0:
                raise ValidationError(_('The serial number has already been assigned: \n Product: %(product)s, Serial Number: %(lot)s', product=product.display_name, lot=lot.name))
        ...
        if product_id.tracking == 'serial':
            ...
            if quants:
                if not source_location_id:
                    ...
                    message = _('The Serial Number (%(lot)s) is already used in these location(s): %(location_list)s.\n\n'
                                ...
                                lot=lot_id.name, loaction_list=format_list(self.env, sn_locations.mapped('display_name')))
                    ...
                    if recommended_location and recommended_location.company_id == company_id:
                        message = _('Serial number (%(lot)s) is not located in %(source_location)s, but is located in location(s): %(other_locations)s.\n\n'
                                    'Source location for this move will be changed to %(recommended_location)s',
                                    lot=lot_id.name,
                                    source_location=source_location_id.display_name,
                                    other_locations=format_list(sn_locations.mapped('display_name')),
