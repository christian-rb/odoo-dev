...
class StockQuant(models.Model):
    ...
    def _domain_product_id(self):
        if self.env.user.has_group('stock.group_stock_user'):
            return ("[] if not context.get('inventory_mode') else"
                " [('is_trackable', '=', True), ('product_tmpl_id', 'in', context.get('product_tmpl_ids', []) + [context.get('product_tmpl_id', 0)])] if context.get('product_tmpl_ids') or context.get('product_tmpl_id') else"
                " [('is_trackable', '=', True)]")
        return "[]"
    ...
    def check_product_id(self):
        if any(not elem.product_id.is_trackable for elem in self):
