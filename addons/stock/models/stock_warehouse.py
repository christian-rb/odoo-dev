    ...
    def write(self, vals):
        ...
        for warehouse in warehouses:
            ...
            if 'active' in vals:
                ...
                if move_ids:
                    raise UserError(_(
                        'You still have ongoing operations for picking types %(type_list)s in warehouse %(warehouse)s',
                        type_list=format_list(self.env, move_ids.mapped('picking_type_id.name')),
                ...
                if picking_type_using_locations:
                    raise UserError(_(
                        '%(type_list)s use default source or destination locations from warehouse %(warehouse)s that will be archived.',
                        type_list=format_list(self.env, picking_type_using_locations.mapped('name')),
                        warehouse=warehouse.name,
                    ))
