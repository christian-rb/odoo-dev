    ...
    def _compute_quantity(self):
        for record in self:
            if not record.quant_id or record.quantity:
                continue
            if float_compare(record.move_id.product_uom_qty, record.move_id.quantity, precision_rounding=record.move_id.product_uom.rounding) > 0:
                record.quantity = max(0, min(record.quant_id.available_quantity, record.move_id.product_uom_qty - record.move_id.quantity))
            else:
