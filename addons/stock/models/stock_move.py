...
Please change the quantity done or the rounding precision of your unit of measure.""",
    ...
    def _push_apply(self):
        new_moves = []
        domain = [('action', 'in', ('push', 'pull_push'))]
        all_rules_domain = expression.AND([domain, [('location_src_id', 'in', self.location_dest_id.ids)]])
        all_rules = self.env['stock.rule']._search_rules(self.route_ids, self.product_packaging_id, self.product_id, self.warehouse_id | self.picking_id.picking_type_id.warehouse_id, all_rules_domain)
        if self.env.company not in self.location_dest_id.company_id:
            all_rules |= self.sudo().env['stock.rule']._search_rules(self.route_ids, self.product_packaging_id, self.product_id, self.warehouse_id | self.picking_id.picking_type_id.warehouse_id, all_rules_domain)
        if all_rules:
            for move in self:
