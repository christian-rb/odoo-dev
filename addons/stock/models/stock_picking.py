...
class Picking(models.Model):
    ...
    def _get_show_allocation(self, picking_type_id):
        ...
        if not picking_type_id or picking_type_id.code == 'outgoing':
            return False
        lines = self.move_ids.filtered(lambda m: m.product_id.is_trackable and m.state != 'cancel')
        if lines:
    ...
    def _check_move_lines_map_quant_package(self, package):
        return package._check_move_lines_map_quant(self.move_line_ids.filtered(lambda ml: ml.package_id == package and ml.product_id.is_trackable))
    ...
    def button_validate(self):
        ...
        if self.env.user.has_group('stock.group_reception_report'):
            ...
            lines = pickings_show_report.move_ids.filtered(lambda m: m.product_id.is_trackable and m.state != 'cancel' and m.quantity and not m.move_dest_ids)
            if lines:
