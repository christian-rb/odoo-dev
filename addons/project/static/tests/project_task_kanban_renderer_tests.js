...
QUnit.module("project", (hooks) => {
    ...
    QUnit.test("delete a column in grouped on m2o", async function (assert) {
        ...
        assert.containsN(target, ".o_kanban_group", 1, "should have 1 column");
        const selectedElement = target.querySelectorAll(".o_kanban_group")[0];
        assert.strictEqual(selectedElement.querySelector(".o_column_title").innerText, "New");
        ...
        assert.containsN(target, ".o_kanban_example_background_container", 1,'Test cases failed')
    });
