...
class TestProjectTaskMailTrackingDuration(MailTrackingDurationMixinCase):
    ...
    def test_task_mail_tracking_duration_during_onchange_stage(self):
        ...
        self.assertEqual(initial_tracking.get(str(self.stage_1.id), False), final_tracking.get(str(self.stage_1.id), False))
        self.assertEqual(final_tracking.get(str(self.stage_2.id)), 0)
