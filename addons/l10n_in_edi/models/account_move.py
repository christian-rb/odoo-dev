...
class AccountMove(models.Model):
    ...
    display_null_pan_warning = fields.Boolean(string="Display warning", compute="_compute_l10n_in_edi_display_null_pan_warning")
    ...
    def _compute_l10n_in_edi_display_null_pan_warning(self):
        for record in self:
            if record.country_code == 'IN':
                tds_tcs_tax_tags = record.env.ref("l10n_in.tax_tag_base_tcs") | record.env.ref("l10n_in.tax_tag_base_tds")
                record.display_null_pan_warning = (
                    not record.partner_id.l10n_in_pan
                    and record.move_type in ['in_invoice', 'out_invoice']
                    and record.amount_residual > 0
                    and record.line_ids.mapped("tax_tag_ids") & tds_tcs_tax_tags
                )
            else:
                record.display_null_pan_warning = False
