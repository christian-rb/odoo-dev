...
var QuestionFormWidget = publicWidget.Widget.extend({
    ...
    _createOrUpdateQuestion: async function (options) {
        ...
        var errorText = validationError.find('.o_wslides_js_quiz_validation_error_text');

        if (!this._isValidForm($form)) {
            validationError.removeClass('d-none');
            errorText.text(_t('Please fill in the question'));
        ...
        if (typeof renderedQuestion === 'object' && renderedQuestion.error) {
            validationError.removeClass('d-none');
            errorText.text(renderedQuestion.error);
            return false;
        }

        validationError.addClass('d-none');
        var event = options && options.update ? 'display_updated_question' : 'display_created_question';
        this.trigger_up(event, {
            newQuestionRenderedTemplate: renderedQuestion,
            $editedQuestion: options && options.update ? this.$editedQuestion : undefined,
            questionFormWidget: this,
        });
        return true;
    },
