...
class StockMove(models.Model):
    ...
    def _create_out_svl(self, forced_quantity=None):
        ...
        for move in self:
            ...
            for line in lines:
                quantities[line.lot_id] += line.product_uom_id._compute_quantity(line.quantity, move.product_id.uom_id)
            if float_is_zero(forced_quantity or sum(quantities.values()), precision_rounding=move.product_id.uom_id.rounding):
                continue

            if move.product_id.lot_valuated:
                vals = []
                for lot_id, lines in lines.grouped('lot_id').items():
                    out_vals = move.product_id._prepare_out_svl_vals(
                        quantities[lot_id],
                        move.company_id,
                        lot_id=lot_id
                    )
                    out_vals['lot_id'] = lot_id.id
                    vals.append(out_vals)
            else:
                vals = [move.product_id._prepare_out_svl_vals(forced_quantity or sum(quantities.values()), move.company_id)]
            for val in vals:
        ...
        for move in self:
            ...
            for line in lines:
                if move.product_id.lot_valuated:
                    ...
                    quantities[self.env['stock.lot']] += line.quantity
            if forced_quantity:
                quantities[self.env['stock.lot']] = forced_quantity
    ...
    def product_price_update_before_done(self, forced_qty=None):
        ...
        for move in self:
            ...
            quantity = sum(quantity_by_lot.values())
            ...
            for lot, qty in quantity_by_lot.items():
                ...
                if float_is_zero(qty_avail, precision_rounding=rounding) \
                        or float_is_zero(qty_avail + move.product_qty, precision_rounding=rounding) \
    ...
    def _get_in_svl_vals(self, forced_quantity):
        ...
        for move in self:
            ...
            if move.product_id.lot_valuated:
                vals = []
                for lot_id, lines in lines.grouped('lot_id').items():
                    vals.append(move.product_id._prepare_in_svl_vals(quantities[lot_id], unit_cost, lot_id=lot_id))
            else:
                vals = [move.product_id._prepare_in_svl_vals(forced_quantity or sum(quantities.values()), unit_cost)]
            for val in vals:
                val.update(move._prepare_common_svl_vals())
                if forced_quantity:
                    val['description'] = _('Correction of %s (modification of past move)', move.picking_id.name or move.name)
