...
class TestSaleTimesheetReport(TestCommonSaleTimesheet):
    ...
    def assertOrderLineReportValues(self, so_line, expected_values_list):
        reports = self.env['timesheets.analysis.report'].search([('so_line', '=', so_line.id)])
        expected_values_dict = [{
            'unit_amount': vals[0],
            'timesheet_revenues': vals[1],
            'billable_time': vals[2],
    ...
    def test_timesheet_report_uom_difference(self):
        ...
        self.assertOrderLineReportValues(so_line, [
            # unit_amount, timesheet_revenues, billable_time
            [          24,                 30,            24],
        ])
