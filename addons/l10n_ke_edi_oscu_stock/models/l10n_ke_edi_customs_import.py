...
class L10nKeEdiCustomsImport(models.Model):
    ...
    _rec_name = 'task_code'
    _rec_names_search = ['task_code', 'item_seq'] # TODO: Not sure if it has much benefit
    ...
    registration_date = fields.Datetime('Registration Date', readonly=True, tracking=True)
    state = fields.Selection([('2', 'Waiting'), ('3', 'Approved'), ('4', 'Rejected')], tracking=True)
    warning_msg = fields.Html(compute='_compute_warning_msg')
    ...
    def _compute_warning_msg(self):
        ...
        for item in product_items:
            warnings = []
            if result := item.product_id._l10n_ke_get_validation_messages(is_invoice=True):
                item.warning_msg = result['message']
                continue

            if item.package_unit_code_id != item.product_id.l10n_ke_packaging_unit_id:
                warnings.append(_('Packaging unit code does not match'))
            if item.origin_country_id != item.product_id.l10n_ke_origin_country_id:
                warnings.append(_('Origin Country is not the same as on the product'))
            if item.uom_id and item.uom_id.category_id != item.product_id.uom_id.category_id:
                warnings.append(_('UoMs do not match'))
            if item.registration_date and item.registration_date < item.write_date:
                warnings.append(_('You might have altered something after the last registration'))
            item.warning_msg = Markup('<br/>').join(warnings)

    def _send_match(self, status='2'):
        if not self.product_id:
            raise UserError(_('Please match with a product first. '))
        if self.product_id._l10n_ke_get_validation_messages(is_invoice=True):
            raise UserError(_('Please handle the warning message first. '))
    ...
    def button_approve(self):
        ...
        self._send_match(status='3')
        # Trigger cron
        self.env.ref("l10n_ke_edi_oscu_stock.ir_cron_send_stock_moves")._trigger()

    def button_reject(self):
        self._send_match(status='4')

    def button_simulate(self):
        with file_open('l10n_ke_edi_oscu_stock/tests/mocked_responses/get_imports_2.json') as import_file:
            test_data = import_file.read()
        data = json.loads(test_data)
        data = data['data']
        self._process_receival_dict(self.env.company, data)
        ...
        for item in data['itemList']:
            ...
            if not custom_item:
                ...
                matched_item = self.search([
                    #('id', '!=', item_rec.id), As product_id is False
                    ('product_id', '!=', False),
                    ('hs_code', '=', item['hsCd']),
    ...
    def _receive_customs_import(self, company):
        """ Receive the different importations"""
        content = {
            'lastReqDt': '20180101000000', #company.l10n_ke_oscu_last_fetch_customs_import_date or
        }
        ...
        company.l10n_ke_oscu_last_fetch_customs_import_date = date
    ...
    def action_create_purchase_order(self):
        ...
        if len(partners) != 1:
            raise UserError(_('You should have only one partner for the Customs Imports. '))
        for custimp in custimps:
            if not custimp.product_id:
                raise UserError(_('Please make sure it has been matched with a product already. '))
