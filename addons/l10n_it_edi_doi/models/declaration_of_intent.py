...
class L10nItDeclarationOfIntent(models.Model):
    ...
    @api.depends('sale_order_ids', 'sale_order_ids.state', 'sale_order_ids.tax_totals')
    ...
    def _build_revoked_warning_message(self):
        self.ensure_one()
        if self.state == 'revoked':
            return _('This document is linked to a revoked Declaration of Intent. Please check the taxes applied.')
        return ''
    ...
    def _check_valid(self, company, partner, date, currency):
        ...
        for declaration in self:
            ...
            if not company or declaration.company_id != company:
                raise ValidationError(_("The Declaration of Intent belongs to company %s, not %s.", declaration.company_id.name, company.name))
            if not currency or declaration.currency_id != currency:
                raise ValidationError(_("The Declaration of Intent uses currency %s, not %s.", declaration.currency_id.name, currency.name))
            if not partner or declaration.partner_id != partner:
                raise ValidationError(_("The Declaration of Intent belongs to partner %s, not %s.", declaration.partner_id.name, partner.name))
            if not date or declaration.start_date > date or declaration.end_date < date:
                raise ValidationError(_("The Declaration of Intent is valid from %s to %s, not on %s.", declaration.start_date, declaration.end_date, date))

    def _is_valid(self, company, partner, date, currency):
        """
        Return whether all declarations of intent in self are valid for the specified `company`, `partner`, `date` and `currency'.
        """
        try:
