...
class SaleOrder(models.Model):
    ...
    l10n_it_edi_doi_intent_warning = fields.Text(
        string="Declaration of Intent Threshold Warning",
        compute='_compute_l10n_it_edi_doi_intent_warning',
    )
    ...
    def _compute_l10n_it_edi_doi_intent_warning(self):
        for order in self:
            order.l10n_it_edi_doi_intent_warning = ''
            ...
            if declaration.state == 'revoked':
                order.l10n_it_edi_doi_intent_warning = declaration._build_revoked_warning_message()
                continue
            ...
            order.l10n_it_edi_doi_intent_warning = declaration._build_threshold_warning_message(declaration_invoiced, declaration_not_yet_invoiced)
    ...
    def _prepare_invoice(self):
        ...
        if declaration:
            ...
            if declaration._is_valid(company, partner, date, currency):
    ...
    def copy_data(self, default=None):
        ...
        for order, data in zip(self, data_list):
            declaration = order.l10n_it_edi_doi_declaration_of_intent_id
            date = fields.Date.context_today(self)
            if not declaration._is_valid(order.company_id, order.partner_id, date, order.currency_id):
