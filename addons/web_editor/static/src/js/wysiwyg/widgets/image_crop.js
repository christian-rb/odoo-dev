...
export class ImageCrop extends Component {
    ...
    async _show(props) {
        ...
        this.imageData = Object.assign({}, registry.category("image.data").get(src, {}));
        this.aspectRatio = this.imageData.aspectRatio || "0/0";
        const isIllustration = /^\/web_editor\/shape\/illustration\//.test(this.imageData.originalSrc);
        this.uncroppable = false;
        if (this.imageData.originalSrc && !isIllustration) {
    ...
    async _save(cropped = true) {
        ...
        [...cropperDataFields, 'aspectRatio'].forEach(attr => {
            delete this.imageData[attr];
            const value = this._getAttributeValue(attr);
            if (value) {
                this.imageData[attr] = value;
            }
        });
        delete this.imageData.resizeWidth;
        this.initialSrc = await applyModifications(this.media, this.imageData, {forceModification: true});
        this.imageData.isCropped = cropped;
