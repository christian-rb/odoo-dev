...
export class OdooEditor extends EventTarget {
    ...
    /**
     * Sets selection in table cells in order to select the cells
    ...
    _setSelectionInTable(ev) {
        ...
        if (selection.rangeCount > 1) {
            ...
            const startRange = getDeepRange(this.editable, { range: selection.getRangeAt(0), correctTripleClick: selection.anchorNode && selection.anchorNode.nodeName === 'TR' });
            const endRange = getDeepRange(this.editable, { range: selection.getRangeAt(selection.rangeCount - 1), correctTripleClick: selection.anchorNode && selection.anchorNode.nodeName === 'TR' });
        ...
        if (startTable !== endTable) {
            ...
            if (endTable) {
                ...
                const isTableFullySelected = getTableCells(endTable)
    ...
    _handleSelectionInTable(ev=undefined) {
        ...
        if ((startTd !== endTd || selectSingleCell) && startTable && startTable === endTable) {
            ...
            const traversedTds = traversedNodes.map(node => closestElement(node, 'td'));
            const isTableFullySelected = endSelectionTableTds?.every(td => traversedTds.includes(td));
            if (endSelectionTable && !isTableFullySelected) {
                ...
                setSelection(selection.anchorNode, selection.anchorOffset, targetTd, 0);
                return;
            }
