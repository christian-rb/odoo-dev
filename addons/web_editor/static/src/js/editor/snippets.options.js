    ...
    start: function () {
        ...
        if (this.canUseHandle) {
            this.$handles.on('mousedown', function (ev) {
                ...
                self.trigger_up("snippet_edition_request", {
                    exec: () => {
...
registry.CarouselHandler = registry.GalleryHandler.extend({
    ...
    _updateIndicatorAndActivateSnippet(position) {
        ...
        for (const indicatorEl of this.$target[0].querySelectorAll(".carousel-indicators img")) {
            indicatorEl.classList.remove("active");
        }
        this.$target[0].querySelector(`.carousel-indicators img[data-bs-slide-to="${position}"]`)
