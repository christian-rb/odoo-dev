...
/**
 * Loads the image data (filter, shape etc...).
 *
...
export async function loadImageInfo(img, recordInfo, attachmentSrc = "") {
    ...
    imageData = await rpc("/web_editor/get_image_info", {
        ...
        src: relativeSrc,
        src_before_hover: img.dataset.srcBeforeHover,
    });
    ...
    if (imageData) {
        ...
        if (img.dataset.srcBeforeHover) {
            // Also linked the image data to the source before an hover effect
            // was applied.
            weUtils.updateImageDataRegistry(img.dataset.srcBeforeHover, imageData);
        }
        return;
    }
