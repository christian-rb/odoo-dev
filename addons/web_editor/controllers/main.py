...
class Web_Editor(http.Controller):
    ...
    def get_image_info(self, res_model, res_id, res_field='', res_type='', search_image_data=True, src=''):
        """Returns the image data (mimetype, image options etc...). If the image
        has never been modified, it returns the mimetype, the image source and
        the id of the original attachment linked to the image so that it can be
        used as a base to modify it (crop/optimization/filters).
    ...
    def modify_image(self, attachment=None, res_model=None, res_id=None, res_field=None, res_type=None, name=None, data=None, mimetype=None, alt_data=None, saved_image_data=None):
        ...
        options linked to the image and returns its image_src to be inserted
        into the DOM.
