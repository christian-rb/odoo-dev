...
const defineModuleSet = async (entryPoints, additionalAddons) => {
    ...
    if (additionalAddons.has("*")) {
        ...
        moduleSet.moduleNames = getModuleNames(addons);
        moduleSet.urlFilter = (url) => addons.has(url.split("/")[1]);
        moduleSet.addons = [...addons];
    }
 ...
 */
const runSuite = async (entryPoint) => {
    const moduleSet = await defineModuleSet([entryPoint], new Set());
    const moduleSetLoader = new ModuleSetLoader(moduleSet);

    const suite = moduleSetLoader.exec(() =>
        describe(getSuitePath(entryPoint), () => {
            moduleSetLoader.startModule(entryPoint + TEST_SUFFIX);
        })
    );
...
const resolveAddonDependencies = (dependencies) => {
    ...
    while ((entry = findJob())) {
        ...
        for (const dep of deps) {
            solved[name].push(dep, ...(solved[dep] || []));
        }
...
const runTests = async () => {
    ...
    for (const moduleName of testModuleNames) {
        if (suitePaths.includes(getSuitePath(moduleName))) {
            const running = await runSuite(moduleName);
            if (!running) {
                break;
            }
        }
...
class ModuleSetLoader extends loader.constructor {
    ...
    constructor(moduleSet) {
        ...
        for (const name of moduleSet.moduleNames) {
     ...
     */
    exec(callback) {
        const checkOdooKeys = watchKeys(window.odoo, WHITE_LISTED_KEYS);
        const checkWindowKeys = watchKeys(window, WHITE_LISTED_KEYS);
        const unwatchListeners = watchListeners();

        // If templates module is available: set URL filter to filter out static
        // templates.
        if (this.moduleSet.urlFilter) {
            this.templateModule?.setUrlFilters([this.moduleSet.urlFilter]);
        }
