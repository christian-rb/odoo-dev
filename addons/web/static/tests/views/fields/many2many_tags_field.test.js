...
import { getActiveElement, hover, press, queryAllTexts, queryOne } from "@odoo/hoot-dom";
...
test("Many2ManyTagsField with and without color", async () => {
    ...
    await clickOpenedDropdownItem("timmy", "gold");
...
test("Many2ManyTagsField with color: rendering and edition", async () => {
    ...
    expect(".o-autocomplete--dropdown-menu li a:eq(0)").toHaveText("red");
...
test("Many2ManyTagsField view a domain", async () => {
    ...
    onRpc("name_search", (args) => {
        expect(args.kwargs.args).toEqual(["&", ["id", "<", 50], "!", ["id", "in", [12]]]);
    });
    ...
    await clickOpenedDropdownItem("timmy", "silver");
...
test("Domain: allow python code domain in fieldInfo", async () => {
    ...
    expect(".o-autocomplete--dropdown-menu li").toHaveCount(3);
    expect(".o-autocomplete--dropdown-menu li a:eq(0)").toHaveText("silver");
    await clickOpenedDropdownItem("timmy", "Start typing...");
...
test("Many2ManyTagsField in editable list", async () => {
    ...
    // add a tag
    await selectDropdownItem("timmy", "silver");
...
test("Many2ManyTagsField keeps focus when being edited", async () => {
    ...
    expect(queryOne("[name=foo] input")).toBe(getActiveElement());

    await selectDropdownItem("timmy", "gold");
    expect(".o_field_many2many_tags .badge").toHaveCount(1);
    expect(queryOne(".o_field_many2many_tags input")).toBe(getActiveElement());
});
...
test("Many2ManyTagsField: quick create a new record", async () => {
    ...
    await runAllTimers();
    await clickOpenedDropdownItem("timmy", `Create "new"`);
    expect(".o_field_many2many_tags .badge").toHaveCount(1);
...
test("Many2ManyTagsField: select multiple records", async () => {
    ...
    await selectDropdownItem("timmy", "Search More...");
...
test("Many2ManyTagsField: save&new in edit mode doesn't close edit window", async () => {
    ...
    await runAllTimers();
    await clickOpenedDropdownItem("timmy", "Create and edit...");
...
test("Many2ManyTagsField: conditional create/delete actions", async () => {
    ...
    await clickDropdown("partner_ids");
    await nameSearchProm;
    await animationFrame();
    expect(
    ...
    await clickOpenedDropdownItem("partner_ids", "Search More...");
...
test("failing many2one quick create in a Many2ManyTagsField", async () => {
    ...
    await runAllTimers();
    await clickOpenedDropdownItem("timmy", `Create "new partner"`);
...
test("navigation in tags (mode 'readonly')", async () => {
    ...
    expect(getActiveElement()).toBe(queryOne(".o_searchview_input"));
    press("ArrowDown");
    press("ArrowDown");
    expect(getActiveElement()).toBe(queryOne("tr.o_data_row input[type=checkbox]"));

    press("ArrowRight");

    expect(getActiveElement()).toBe(queryOne("tr.o_data_row td[name=timmy]"));
});
...
test("navigation in tags (mode 'edit')", async () => {
    ...
    expect(getActiveElement()).toBe(
        queryOne("tr.o_data_row:eq(0) [name=timmy] .o-autocomplete--input")
    );
    ...
    expect(getActiveElement()).toBe(
        queryOne("tr.o_data_row:eq(0) [name=timmy] .badge:nth-child(2)")
    );
    ...
    expect(getActiveElement()).toBe(
        queryOne("tr.o_data_row:eq(0) [name=timmy] .badge:nth-child(1)")
    );
...
test("Many2ManyTagsField with arch context in form view", async () => {
    ...
    await selectDropdownItem("timmy", "gold coucou");
...
test("Many2ManyTagsField doesn't use virtualId for 'name_search'", async () => {
    onRpc("name_search", ({ kwargs }) => {
        ...
        expect(kwargs.args).toEqual(["!", ["id", "in", [2]]]);
    });
    await mountView({
        type: "form",
        resModel: "partner",
