...
import { getActiveElement, press, queryAllTexts, queryFirst, queryOne } from "@odoo/hoot-dom";
...
test("widget many2many_tags_avatar list view - don't crash on keyboard navigation", async () => {
    ...
    press("arrowleft");
    expect(queryFirst(".o_data_row:nth-child(2) .o_field_tags span:nth-child(2)")).toBe(
        getActiveElement()
    );

    // Pressing left arrow again should not crash and should focus on the first tag.
    press("arrowleft");
    expect(queryFirst(".o_data_row:nth-child(2) .o_field_tags span:nth-child(1)")).toBe(
        getActiveElement()
    );
});
...
test("widget many2many_tags_avatar in kanban view", async () => {
    ...
    expect(queryOne(".o-overlay-container input")).toBe(getActiveElement());
