...
export class ActionMenus extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            ...
            this.printItems = await this.getPrintItems(this.props);
        });
        onWillUpdateProps(async (nextProps) => {
            this.actionItems = await this.getActionItems(nextProps);
            this.printItems = await this.getPrintItems(nextProps);
        });
    }
        ...
        const filteredPrintActions = await asyncFilter(printActions, async (printAction) => {
            if (printAction.domain) {
                const recMatch = await this.orm.search(
                    this.props.resModel,
                    [["id", "in", this.props.getActiveIds()], ...new Domain(printAction.domain).toList()],
                    { limit: 1 },
                );
