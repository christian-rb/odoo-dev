...
export function makeActionManager(env, router = _router) {
    ...
    function _getActionInfo(action, props) {
        ...
        actionProps.updateActionState = (controller, newState) => {
            const copyState = { ...currentState, ...newState };
            const changed = !shallowEqual(currentState, copyState);
            Object.assign(currentState, copyState);
    ...
    function _getViewInfo(view, action, views, props = {}) {
        ...
        if (view.type === "form") {
            viewProps.updateActionState = (controller, newState) => {
                const copyState = { ...currentState, ...newState };
                const changed = !shallowEqual(currentState, copyState);
                Object.assign(currentState, copyState);
            ...
            get componentProps() {
                ...
                if (updateActionState) {
                    componentProps.updateActionState = (newState) =>
                        updateActionState(this, newState);
                }
                if (this.constructor.Component === View) {
