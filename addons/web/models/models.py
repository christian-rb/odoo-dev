...
class Base(models.AbstractModel):
    ...
    def web_read_group(self, domain, groupby, aggregates, limit=None, offset=0, order='', fill_temporal=None):
        ...
        complete_order = order + ','.join(groupby)
        ...
        if groupby and (fill_temporal or isinstance(fill_temporal, dict)):
            ...
            if not isinstance(fill_temporal, dict):
                fill_temporal = {}
            groups = self._web_read_group_fill_temporal(groups, groupby, aggregates, order, **fill_temporal)
            length = len(groups)

        # TODO Should we have an alternative without lenght ??
        return {
    ...
    @api.model
    def _web_read_group_fill_temporal(self, groups, groupby, aggregates, order, fill_from=False, fill_to=False):
        ...
        granularity = first_group.split(':')[1]
        days_offset = 0
        if granularity == 'week':
        ...
        if (first_group + ' desc') in order.lower():
            required_dates = reversed(tuple(required_dates))
            if any(not group_value for group_value, *__ in groups):
                required_dates = (False,) + tuple(required_dates)

        result = dict.fromkeys(required_dates, empty_item)
        result.update({
            group_value: other_values
            for group_value, *other_values in groups
        })
        return [(value,) + aggregate_values for value, aggregate_values in result.items()]
    ...
    def _web_read_group_format_groupby(self, groupby_spec, values):
        ...
        for raw_value in values:
            ...
            if not raw_value and field.type == 'many2many':
                ...
                # TODO: Should the label be created by the webclient ?
                value = (range_start, label)
                additional_domain = [
