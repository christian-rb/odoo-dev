...
class Users(models.Model):
    ...
    def _totp_check(self, code):
        ...
        if match is None:
            _logger.info("%r 2FA check: FAIL for %s %r", LogType.MFA_LOGIN_FAIL, self, sudo.login)
            raise AccessDenied(_("Verification failed, please double-check the 6-digit code"))
        _logger.info("%r 2FA check: SUCCESS for %s %r", LogType.MFA_LOGIN_SUCCESS, self, sudo.login)

    def _totp_try_setting(self, secret, code):
        if self.totp_enabled or self != self.env.user:
            _logger.info("%r 2FA enable: REJECT for %s %r", LogType.MFA_ACTIVATION_FAIL, self, self.login)
            return False

        secret = compress(secret).upper()
        match = TOTP(base64.b32decode(secret)).match(code)
        if match is None:
            _logger.info("%r 2FA enable: REJECT CODE for %s %r", LogType.MFA_ACTIVATION_FAIL, self, self.login)
            return False
        ...
        _logger.info("%r 2FA enable: SUCCESS for %s %r", LogType.MFA_ACTIVATION_SUCCESS, self, self.login)
        return True
    ...
    def action_totp_disable(self):
        ...
        if not (self == self.env.user or self.env.user._is_admin() or self.env.su):
            _logger.info("%r 2FA disable: REJECT for %s (%s) by user %s (#%d)", LogType.MFA_DEACTIVATION_FAIL, self,
                         logins, self.env.user.login, self.env.user.id)
            return False
        ...
        _logger.info("%r 2FA disable: SUCCESS for %s (%s) by user %s (#%d)", LogType.MFA_DEACTIVATION_SUCCESS, self,
         logins, self.env.user.login, self.env.user.id)
        return {
    ...
    def _revoke_all_devices(self):
        ...
        _logger.info("%r All TOTP devices revoked for %s (#%s) by user %s (#%d)", LogType.TOTP_DEVICE_REMOVED,
