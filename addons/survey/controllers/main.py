...
class Survey(http.Controller):
    ...
    def _prepare_survey_data(self, survey_sudo, answer_sudo, **post):
        ...
        if answer_sudo.state == 'new' and len(langs := request.env['res.lang'].sudo().get_installed()) > 1:
    ...
    def survey_display_page(self, survey_token, answer_token, **post):
        if (lang_code := post.get('lang_code')) and request.httprequest.method == 'POST':
            return request.redirect(url_for(f'/survey/{survey_token}/{answer_token}', lang_code))
