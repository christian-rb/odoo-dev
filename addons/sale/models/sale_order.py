...
class SaleOrder(models.Model):
    ...
    def _get_invoiced(self):
        ...
        for order in self:
            invoices = order.order_line.move_line_ids.move_id.filtered(lambda r: r.move_type in ('out_invoice', 'out_refund'))
    ...
    def _search_invoice_ids(self, operator, value):
        if operator == 'in' and value:
            ...
            #
            # We cannot just search [('order_line.move_line_ids', '=', False)]
            ...
            #
            # Domain below returns subset of ('order_line.move_line_ids', '!=', False)
            order_ids = self._search([
                ('order_line.move_line_ids.move_id.move_type', 'in', ('out_invoice', 'out_refund'))
            ])
            return [('id', 'not in', order_ids)]
        return [
            ('order_line.move_line_ids.move_id.move_type', 'in', ('out_invoice', 'out_refund')),
            ('order_line.move_line_ids.move_id', operator, value),
        ]
            ...
            if final:
                ...
                for order_line in self.order_line:
                    if not order_line.is_downpayment:
                        continue
                    inv_amt = order_amt = 0
                    for invoice_line in order_line.move_line_ids:
