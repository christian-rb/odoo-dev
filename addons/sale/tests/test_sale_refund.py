...
class TestSaleRefund(TestSaleCommon):
    ...
    def test_refund_create(self):
        ...
        for line in self.sale_order.order_line:
            if line.product_id.invoice_policy == 'delivery':
                ...
                self.assertFalse(line.move_line_ids, "The line based on delivered qty are not invoiced, so they should not be linked to invoice line")
            else:
                ...
                self.assertEqual(len(line.move_line_ids), 1, "The lines 'ordered' qty are invoiced, so it should be linked to 1 invoice lines")
        ...
        for line in self.sale_order.order_line:
            if line.product_id.invoice_policy == 'delivery':
                ...
                self.assertEqual(line.untaxed_amount_to_invoice, 0.0, "The amount to invoice should be zero, as the line based on delivered quantity")
                self.assertEqual(line.untaxed_amount_invoiced, 0.0, "The invoiced amount should be zero, as the line based on delivered quantity")
                self.assertFalse(line.move_line_ids, "The line based on delivered are not invoiced, so they should not be linked to invoice line")
            else:
                if line == self.sol_prod_order:
                    ...
                    self.assertEqual(len(line.move_line_ids), 2, "The line 'ordered consumable' is invoiced, so it should be linked to 2 invoice lines (invoice and refund)")
                else:
                    ...
                    self.assertEqual(len(line.move_line_ids), 2, "The line 'ordered service' is invoiced, so it should be linked to 2 invoice lines (invoice and refund)")
    ...
    def test_refund_modify(self):
        ...
        for line in self.sale_order.order_line:
            if line.product_id.invoice_policy == 'delivery':
                ...
                if line == self.sol_prod_order:
                    ...
                    self.assertEqual(len(line.move_line_ids), 3, "The line 'ordered consumable' is invoiced, so it should be linked to 3 invoice lines (invoice and refund)")
                else:
                    ...
                    self.assertEqual(line.untaxed_amount_invoiced, 0.0, "Amount invoiced is zero as the invoice 1 and its refund are reconcilied")
                    self.assertEqual(len(line.move_line_ids), 3, "The line 'ordered service' is invoiced, so it should be linked to 3 invoice lines (invoice and refund)")
        ...
        for line in self.sale_order.order_line:
            if line.product_id.invoice_policy == 'delivery':
                ...
                self.assertEqual(line.untaxed_amount_to_invoice, 0.0, "The amount to invoice should be zero, as the line based on delivered quantity")
                self.assertEqual(line.untaxed_amount_invoiced, 0.0, "The invoiced amount should be zero, as the line based on delivered quantity")
                self.assertFalse(line.move_line_ids, "The line based on delivered are not invoiced, so they should not be linked to invoice line, even after validation")
            else:
                if line == self.sol_prod_order:
                    ...
                    self.assertEqual(len(line.move_line_ids), 3, "The line 'ordered consumable' is invoiced, so it should be linked to 2 invoice lines (invoice and refund), even after validation")
                else:
                    ...
                    self.assertEqual(len(line.move_line_ids), 3, "The line 'ordered service' is invoiced, so it should be linked to 2 invoice lines (invoice and refund), even after validation")
    ...
    def test_refund_invoice_with_downpayment(self):
        ...
        self.assertEqual(len(sol_product.move_line_ids), 2, "The product line is invoiced, so it should be linked to 2 invoice lines (invoice and refund)")
        ...
        self.assertEqual(len(sol_downpayment.move_line_ids), 3, "The product line is invoiced, so it should be linked to 3 invoice lines (downpayment invoice, partial invoice and refund)")
