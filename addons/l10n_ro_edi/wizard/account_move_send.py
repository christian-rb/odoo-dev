...
class AccountMoveSend(models.TransientModel):
    ...
    def _compute_l10n_ro_edi_send_enable(self):
        for wizard in self:
            wizard.l10n_ro_edi_send_enable = any(
                (m._need_ubl_cii_xml() or m.ubl_cii_xml_id) and m.l10n_ro_edi_state not in ('ok', 'sent')
    ...
    def _compute_l10n_ro_edi_send_readonly(self):
        for wizard in self:
            wizard.l10n_ro_edi_send_readonly = not wizard.l10n_ro_edi_send_enable or any(
                move.l10n_ro_edi_state in ('sending', 'ok', 'sent') for move in wizard.move_ids)
    ...
    def _compute_l10n_ro_edi_send_warning_message(self):
        for wizard in self:
            ...
            moves_in_sending = wizard.move_ids.filtered(lambda m: m.l10n_ro_edi_state == 'sending').mapped('name')
            if moves_in_sending:
                warning_messages = _("The following move(s) are waiting for answer from E-Factura: %s", ', '.join(moves_in_sending))
            self.l10n_ro_edi_send_warning_message = warning_messages
    ...
    def _call_web_service_after_invoice_pdf_render(self, invoices_data):
        ...
        for invoice, invoice_data in invoices_data.items():
            if invoice_data.get('l10n_ro_edi_send') and invoice.l10n_ro_edi_state not in ('sending', 'done'):
                ...
                if errors := invoice._l10n_ro_edi_prepare_values(xml_data):
                    invoice_data['error'] = {
                ...
                document = self.env['ciusro.document'].create({'move_id': invoice.id})
