...
class PurchaseEdiXmlUBLBIS3(models.AbstractModel):
    ...
    def _get_payment_terms_vals_list(self, payment_term):
        return {
    ...
    def _get_order_lines(self, order):
        def _get_order_line_vals(order_line):
            return {
                'id': order_line.sequence,
                'quantity': order_line.product_qty,
                'quantity_unit_code': self._get_uom_unece_code(order_line),
        ...
        order_lines = [_get_order_line_vals(line) for line in order.order_line]
        for line_id, line in enumerate(order_lines, 1):
            line['id'] = line_id
        return order_lines
    ...
    def _export_order_vals(self, order):
        ...
        vals = {
            ...
            'vals': {
                ...
                'payment_terms_vals': self._get_payment_terms_vals_list(order.payment_term_id),
