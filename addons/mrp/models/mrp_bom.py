...
class MrpBom(models.Model):
    ...
    def _get_extra_attachments(self):
        final_domain = []
        bom_domain = ['&', ('attached_on_mrp', '=', 'bom'), ('active', '=', 't')]
        ...
        for bom in self:
            ...
            if is_byproduct:
                product_domain = OR([product_subdomain, [
                    '|',
                    '&', ('res_model', '=', 'product.product'), ('res_id', '=', bom.byproduct_ids.product_id.id),
                    '&', ('res_model', '=', 'product.template'), ('res_id', '=', bom.byproduct_ids.product_id.product_tmpl_id.id)]])
            else:
