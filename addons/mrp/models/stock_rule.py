...
class StockRule(models.Model):
    ...
    def _get_matching_bom(self, product_id, company_id, values):
        ...
        return self.env['mrp.bom']._bom_find(product_id, picking_type=self.picking_type_id, bom_type='normal', company_id=company_id.id)[product_id]  # no change
    ...
    def _get_lead_days(self, product, **values):
        ...
        bom = values.get('bom') or self.env['mrp.bom']._bom_find(product, picking_type=manufacture_rule.picking_type_id, company_id=manufacture_rule.company_id.id)[product]  # no change
...
class ProcurementGroup(models.Model):
    ...
    def run(self, procurements, raise_user_error=True):
        ...
        # }
        kit_boms = self.env['mrp.bom']._boms_find(procurements, bom_type='phantom')
        for procurement in procurements:
            # bom_kit = kits_by_company[procurement.company_id].get(procurement.product_id)
            bom_kit = kit_boms[procurement.product_id].filtered(lambda b: not b.company or b.company == procurement.company_id)[:1]
            if bom_kit:
