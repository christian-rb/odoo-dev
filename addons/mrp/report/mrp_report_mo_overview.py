...
class ReportMoOverview(models.AbstractModel):
    ...
    def _get_report_data(self, production_id):
        ...
        if production.bom_id:
            ...
            missing_components = (bom_line for bom_line in production.bom_id.bom_line_ids if bom_line not in production.move_raw_ids.bom_line_id)
            ...
            for line in missing_operations:
                line_cost = line._compute_expected_operation_cost()
                initial_bom_cost += currency.round(line_cost * production.product_uom_qty / production.bom_id.product_qty)
        ...
        summary = self._get_mo_summary(production, components, initial_mo_cost, initial_bom_cost, initial_real_cost, remaining_cost_share)
    ...
    def _get_report_extra_lines(self, summary, components, operations, production):
        ...
        if self._is_production_started(production):
            ...
            unit_mo_cost_decorator = self._get_comparison_decorator(unit_bom_cost, unit_mo_cost, currency.rounding)
        extras = {
            'unit_mo_cost': unit_mo_cost,
            'unit_mo_cost_decorator': unit_mo_cost_decorator,
            'unit_bom_cost': unit_bom_cost,
            'unit_real_cost': unit_real_cost,
            # 'unit_real_cost_decorator': self._get_comparison_decorator(unit_mo_cost, unit_real_cost, currency.rounding), not used?
        }
        if production.state == 'done':
            ...
            extras['total_real_cost_operations_decorator'] = self._get_comparison_decorator(extras['total_mo_cost_operations'], extras['total_real_cost_operations'], currency.rounding)
            ...
            # extras['total_mo_cost_decorator'] = self._get_comparison_decorator(extras['total_real_cost'], extras['total_mo_cost'], currency.rounding) not used?
            extras['total_real_cost_decorator'] = self._get_comparison_decorator(extras['total_mo_cost'], extras['total_real_cost'], currency.rounding)
        return extras
    ...
    def _get_mo_summary(self, production, components, current_mo_cost, current_bom_cost, current_real_cost, remaining_cost_share):
        currency = (production.company_id or self.env.company).currency_id
    ...
    # Returns success if current < expected, danger if current > expected
    def _get_comparison_decorator(self, expected, current, rounding):
        ...
        if float_is_zero(current, precision_rounding=rounding) or compare == 0 or expected is False or current is False:
    ...
    def _get_bom_operation_cost(self, workorder, production):
        ...
        return workorder._compute_expected_operation_cost() / (workorder.duration_expected or 1) * bom_duration_expected
    ...
    def _get_operations_data(self, production, level=0, current_index=False):
        ...
        return {
            'summary': {
                ...
                'mo_cost_decorator': self._get_comparison_decorator(total_bom_cost, total_expected_cost, 0.01) if not self._is_production_started(production) else False,
    ...
    def _get_finished_operation_data(self, production, level=0, current_index=False):
        ...
        return {
            'summary': {
                ...
                'real_cost_decorator': self._get_comparison_decorator(total_bom_cost, total_cost, currency.rounding),
                'uom_name': done_operation_uom,
                'currency_id': currency.id,
                'currency': currency,
            },
    ...
    def _format_component_move(self, production, move_raw, replenishments, replenish_data, level, index):
        ...
        replenish_mo_cost, dummy_bom_cost, dummy_real_cost = self._compute_cost_sums(replenishments)
        ...
        real_cost = currency.round(self._get_component_real_cost(move_raw, current_quantity))
        if production.bom_id:
    ...
    def _get_replenishment_lines(self, production, move_raw, replenish_data, level, current_index):
        ...
        for count, forecast_line in enumerate(current_lines):
            ...
            replenishment['summary'] = {
                ...
                'bom_cost': currency.round(self._get_component_real_cost(move_raw, bom_quantity)),
