...
class StockPickingBatch(models.Model):
    ...
    def _compute_picking_info(self):
        for record in self:
            ...
            max_weight = f"{round(record.max_picking_weight, 2)}kg" if record.max_picking_weight else '0Kg'
            max_volume = f"{round(record.max_picking_weight, 2)}m³" if record.max_picking_volume else '0m³'
