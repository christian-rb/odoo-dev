...
class AccountMove(models.Model):
    ...
    def l10n_in_edi_ewaybill_send(self):
        ...
        for move in self:
            ...
            if errors:
                invoice_line = self.invoice_line_ids.filtered(lambda line: (not line.l10n_in_hsn_code or not line.tax_ids))
                action_error = {
                   'view_mode': 'tree',
                ...
                raise RedirectWarning(_("Unable to send Ewaybill:\n\n%s", '\n'.join(errors)), action_error, _('View lines without HSN/Tax'))
