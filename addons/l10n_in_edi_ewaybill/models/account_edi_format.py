    ...
    def _l10n_in_edi_ewaybill_irn_post_invoice_edi(self, invoices):
        ...
        if not response.get("error"):
            ...
            """).format(
                _('E-wayBill Sent'),
                _('Number'),
                str(response.get("data", 0).get('ewayBillNo', 0)) or str(response.get("data", 0).get('EwbNo', 0)),
                _('Validity'),
    ...
    def _l10n_in_edi_ewaybill_post_invoice_edi(self, invoices):
        ...
        if not response.get("error"):
            ...
            """).format(
                _('E-wayBill Sent'),
                _('Number'),
                str(response.get("data", False).get('ewayBillNo', 0)) or str(response.get("data", False).get('EwbNo', 0)),
                _('Validity'),
                str(response.get("data").get('EwbValidTill'))
            )
            invoices.message_post(body=body)
