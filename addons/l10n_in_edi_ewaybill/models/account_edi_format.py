...
class AccountEdiFormat(models.Model):
    ...
    def _check_move_configuration(self, move):
        ...
        invoice_lines = move.invoice_line_ids.filtered(lambda line: not (line.display_type in ('line_section', 'line_note', 'rounding')))
        if any(not line.l10n_in_hsn_code for line in invoice_lines):
            error_message.append(_("HSN/SAC code is missing on Invoice Line"))
        if any(not line.tax_ids for line in invoice_lines):
            error_message.append(_("Tax is missing on Invoice Line"))
        if all(line.product_id.type == "service" for line in invoice_lines if line.product_id):
