...
class AccountMove(models.Model):
    ...
    l10n_vn_edi_state = fields.Selection(
        ...
        copy=False,
        compute='_compute_l10n_vn_edi_state',
        store=True,
        readonly=False,
    )
    ...
    l10n_vn_edi_sinvoice_file_id = fields.Many2one(
        ...
        depends=['l10n_vn_edi_sinvoice_file_id'],
        export_string_translation=False,
    )
    ...
    l10n_vn_edi_sinvoice_xml_file_id = fields.Many2one(
        ...
        depends=['l10n_vn_edi_sinvoice_xml_file_id'],
        export_string_translation=False,
    )
    ...
    l10n_vn_edi_sinvoice_pdf_file_id = fields.Many2one(
        ...
        depends=['l10n_vn_edi_sinvoice_pdf_file_id'],
        export_string_translation=False,
    )
    ...
    @api.depends('l10n_vn_edi_state', 'state')
    def _compute_show_reset_to_draft_button(self):
        # EXTEND 'account'
        super()._compute_show_reset_to_draft_button()
        self.filtered(lambda m: m.l10n_vn_edi_invoice_number and m.l10n_vn_edi_state != 'cancelled').show_reset_to_draft_button = False

    @api.depends('l10n_vn_edi_state')
    def _compute_need_cancel_request(self):
        # EXTEND 'account' to add dependencies
        return super()._compute_need_cancel_request()

    @api.depends('payment_state')
    def _compute_l10n_vn_edi_state(self):
        ...
        """
        for invoice in self:
            if invoice.l10n_vn_edi_state == 'sent':
                # This is a bit simplistic, as it can be wrongly set (for example, no need to send when going from in_payment to paid)
                # But this shouldn't be an issue since the logic to send the update will check if anything need to change.
                invoice.l10n_vn_edi_state = 'payment_state_to_update'
            else:
                invoice.l10n_vn_edi_state = invoice.l10n_vn_edi_state
    ...
    def button_request_cancel(self):
        # EXTEND 'account'
        if self._need_cancel_request() and self.l10n_vn_edi_invoice_number:
            return {
                'name': _('Invoice Cancellation'),
                'type': 'ir.actions.act_window',
                'view_type': 'form',
                'view_mode': 'form',
                'res_model': 'l10n_vn_edi.cancellation',
                'target': 'new',
    ...
    def _l10n_vn_edi_get_file_data(self, file_format):
        ...
        request_response, error_message = self._l10n_vn_edi_send_request(
            ...
            json_data={
                ...
                'transactionUuid': self.l10n_vn_edi_transaction_id,
    ...
    def _l10n_vn_edi_get_xml_data(self):
        """
        Query sinvoice in order to fetch the xls and xml data representation of the invoice.
        ...
        # The content of the inner zip is a xls file as well as a xml file.
    ...
    def _l10n_vn_edi_get_pdf_data(self):
        """
        ...
        """
        file_data = self._l10n_vn_edi_get_file_data('PDF')

        # Sometimes the documents are not available right away. This is quite rare, but I saw it happen a few times.
        # To handle that we will try up to three time to fetch the document => The impact should be negligible.
        threshold = 1
        while not file_data['fileToBytes'] and threshold < 3:
            time.sleep(0.125 * threshold)
            file_data = self._l10n_vn_edi_get_file_data('PDF')
            threshold += 1
        ...
        for invoice in self:
            ...
            if invoice.payment_state in {'in_payment', 'paid'} and sinvoice_status == 'unpaid':
                ...
                params = {
                    ...
                    'templateCode': invoice.l10n_vn_edi_invoice_symbol.invoice_template_id.name,
            ...
            if endpoint and params:
                ...
                invoice.l10n_vn_edi_state = 'sent'

    def _need_cancel_request(self):
        # EXTEND 'account'
        return super()._need_cancel_request() or self.l10n_vn_edi_invoice_number
    ...
    def _post(self, soft=True):
        ...
        for invoice in posted.filtered(lambda i: not i.l10n_vn_edi_state):
            if invoice.company_id.account_fiscal_country_id.code == 'VN' and invoice.is_sale_document() and not invoice.l10n_vn_edi_invoice_number:
                invoice.l10n_vn_edi_state = 'ready_to_send'
    ...
    def _l10n_vn_edi_check_invoice_configuration(self):
        ...
        if company.phone and not company.phone.replace(' ', '').isdecimal():
            errors.append(_('Phone number for company %s must only contain digits.', company.display_name))
        if commercial_partner.phone and not commercial_partner.phone.replace(' ', '').isdecimal():
            errors.append(_('Phone number for partner %s must only contain digits.', commercial_partner.display_name))
        ...
        if self.move_type == 'out_refund' and (not self.reversed_entry_id or not self.reversed_entry_id.l10n_vn_edi_invoice_number):
        ...
        if not self.partner_id.street or not self.partner_id.city or not self.partner_id.state_id or not self.partner_id.country_id:
            errors.append(_('The street, city, state and country of partner %s must be provided.', company.display_name))
        if not company.street or not company.state_id or not company.country_id:
            errors.append(_('The street, state and country of company %s must be provided.', self.partner_id.display_name))
        return errors
    ...
    def _l10n_vn_edi_send_invoice(self, invoice_json_data):
        ...
        if self.l10n_vn_edi_transaction_id:
            ...
            self.l10n_vn_edi_transaction_id = invoice_json_data['generalInvoiceInfo']['transactionUuid']
    ...
    def _l10n_vn_edi_cancel_invoice(self, reason, agreement_document_name, agreement_document_date):
        ...
        self.l10n_vn_edi_state = 'cancelled'

    def button_draft(self):
        # OVERRIDE account
        super().button_draft()
        # When going from cancelled => draft, we ensure to clear the edi fields so that the invoice can be resent if required.
        self.write({
            'l10n_vn_edi_transaction_id': False,
            'l10n_vn_edi_invoice_number': False,
            'l10n_vn_edi_reservation_code': False,
            'l10n_vn_edi_issue_date': False,
            'l10n_vn_edi_state': False,
        })
    ...
    def _l10n_vn_edi_generate_invoice_json(self):
        ...
        return {
            'generalInvoiceInfo': self._l10n_vn_edi_get_general_invoice_information(),
            'buyerInfo': self._l10n_vn_edi_get_buyer_information(),
            'sellerInfo': self._l10n_vn_edi_get_seller_information(),
            'payments': self._l10n_vn_edi_get_payment_information(),
            'itemInfo': self._l10n_vn_edi_get_item_information(),
            'taxBreakdowns': self._l10n_vn_edi_get_tax_breakdowns(),
        }

    def _l10n_vn_edi_get_general_invoice_information(self):
    ...
    def _l10n_vn_edi_get_buyer_information(self):
        ...
        buyer_information = {
            ...
            'buyerPhoneNumber': self.commercial_partner_id.phone.replace(' ', '') if self.commercial_partner_id.phone else '',
    ...
    def _l10n_vn_edi_get_seller_information(self):
        ...
        seller_information = {
            ...
            'sellerPhoneNumber': self.company_id.phone.replace(' ', '') if self.company_id.phone else '',
    ...
    def _l10n_vn_edi_get_payment_information(self):
    ...
    def _l10n_vn_edi_get_item_information(self):
    ...
    def _l10n_vn_edi_get_tax_breakdowns(self):
        ...
        for tax_percentage, tax_percentage_values in tax_details_grouped['tax_details'].items():
            tax_breakdowns.append({
                ...
                'taxableAmount': tax_percentage_values['base_amount'],
                'taxAmount': tax_percentage_values['tax_amount'],
