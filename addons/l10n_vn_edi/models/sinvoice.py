...
class SInvoiceTemplate(models.Model):
    ...
    def _constrains_changes(self):
        ...
        """
        # As we don't store the template on the invoice, we will first read group by symbol then group by template.
        invoice_counts = self.env['account.move']._read_group(
            domain=[
                ('l10n_vn_edi_invoice_symbol.invoice_template_id', 'in', self.ids),
                ('l10n_vn_edi_state', 'not in', ('ready_to_send', False)),  # Only matches sent invoices.
            ],
            groupby=['l10n_vn_edi_invoice_symbol'],
            aggregates=['__count'],
        )
        invoices_per_template = defaultdict(int)
        for symbol, count in invoice_counts:
            invoices_per_template[symbol.invoice_template_id.id] += count

        for record in self:
            if invoices_per_template[record.id] > 0:
                raise UserError(_('You cannot change the code or type of the template %s because it has '
