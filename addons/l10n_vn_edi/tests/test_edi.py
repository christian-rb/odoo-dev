...
class TestVNEDI(AccountTestInvoicingCommon):
    ...
    def test_invoice_creation(self):
        ...
        self.assertFalse(invoice.l10n_vn_edi_state)  # State should be False before posting.
        self.assertEqual(invoice.l10n_vn_edi_invoice_symbol.id, self.symbol.id)
        invoice.action_post()
        self.assertEqual(invoice.l10n_vn_edi_state, 'ready_to_send')
    ...
    def test_send_and_print(self):
        ...
        with patch('odoo.addons.l10n_vn_edi.models.account_move.AccountMove._l10n_vn_edi_get_pdf_data', return_value=pdf_response), \
             patch('odoo.addons.l10n_vn_edi.models.account_move.AccountMove._l10n_vn_edi_get_xml_data', return_value=xml_response), \
             patch('odoo.addons.l10n_vn_edi.models.account_move.AccountMove._l10n_vn_edi_send_request', return_value=(request_response, None)):
        ...
        self.assertIsNotNone(invoice.l10n_vn_edi_sinvoice_xml_file)
        self.assertIsNotNone(invoice.l10n_vn_edi_sinvoice_pdf_file)
        self.assertIsNotNone(invoice.l10n_vn_edi_sinvoice_file)
    ...
    def test_cancel_invoice(self):
        ...
        invoice.l10n_vn_edi_invoice_number = 'K24TUT01'
        ...
        self.assertEqual(action['res_model'], 'l10n_vn_edi.cancellation')
        with patch('odoo.addons.l10n_vn_edi.models.account_move.AccountMove._l10n_vn_edi_send_request', return_value=(None, None)):
            self.env['l10n_vn_edi.cancellation'].create({
                'invoice_id': invoice.id,
                'reason': 'Unwanted',
                'agreement_document_name': 'N/A',
                'agreement_document_date': fields.Datetime.now(),
            }).button_request_cancel()
        self.assertEqual(invoice.l10n_vn_edi_state, 'cancelled')
