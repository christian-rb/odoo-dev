...
class SaleOrder(models.Model):
    ...
    @api.depends('order_line', 'amount_total', 'order_line.move_line_ids.parent_state',
                 'order_line.move_line_ids.price_total', 'order_line.pos_order_line_ids')
    def _compute_amount_unpaid(self):
        for sale_order in self:
            total_invoice_paid = sum(sale_order.order_line.filtered(lambda l: not l.display_type).mapped(
                'move_line_ids').filtered(lambda l: l.parent_state != 'cancel').mapped('price_total'))
