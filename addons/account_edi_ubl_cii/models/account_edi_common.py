...
class AccountEdiCommon(models.AbstractModel):
    ...
    def _import_partner(self, invoice, logs, vat, phone, email, name, country_code):
    ...
    def _import_document_allowance_charges(self, tree, invoice, qty_factor, logs):
        xpaths = self._get_document_allowance_charge_xpaths()
        line_vals = []
        for allow_el in tree.iterfind(xpaths['root']):
            name = allow_el.findtext(xpaths['reason'], default='')
            ...
            amount = float(allow_el.findtext(xpaths['amount'], default='0'))
            base_amount = float(allow_el.findtext(xpaths['base_amount'], default='0'))
            if base_amount:
                price_unit = base_amount * charge_indicator * qty_factor
                percentage = float(allow_el.findtext(xpaths['percentage'], default='100'))
            ...
            for tax_percent_node in allow_el.iterfind(xpaths['tax_percentage']):
                ...
                if tax:
                    ...
                    logs.append(
                        _("Could not retrieve the tax: %s %% for line '%s'.", tax_amount, name)
                    )
    ...
    def _import_currency(self, invoice, currency_node, logs):
        if currency_node is not None:
            currency = self.env['res.currency'].with_context(active_test=False).search([
                ('name', '=', currency_node.text),
            ], limit=1)
            if currency:
                ...
                logs.append(_("Could not retrieve currency: %s. Did you enable the multicurrency option "
                              "and activate the currency?", currency_node.text))
    ...
    def _import_narration(self, invoice, tree, xpaths):
        ...
        for xpath in xpaths:
            ...
            if node is not None and node.text:
                narration += f"<p>{html_escape(node.text)}</p>"
        invoice.narration = narration

    def _import_prepaid_amount(self, invoice, tree, xpath, qty_factor, logs):
        prepaid_amount = float(tree.findtext(xpath, default='0'))
    ...
    def _import_invoice_lines(self, invoice, tree, xpath, qty_factor, logs):
        for line_tree in tree.iterfind(xpath):
            ...
            self._retrieve_taxes(invoice_line, line_values, logs)
    ...
    def _retrieve_invoice_line_vals(self, tree, invoice_line, qty_factor):
        ...
        # basis_qty (optional)
        basis_qty = float(self._retrieve_first_value_in_tree(tree, xpaths=xpath_dict['basis_qty']) or 1)
        ...
        # check compatibility product and uom
        ...
        charges = []
        discount_amount = 0
        for allw_chrg_node in tree.iterfind(xpath_dict['allowance_charge']):
            charge_indicator = allw_chrg_node.findtext(xpath_dict['allowance_charge_indicator'])
            amount = float(allw_chrg_node.findtext(xpath_dict['allowance_charge_amount'], default='0'))
            reason_code = allw_chrg_node.findtext(xpath_dict['allowance_charge_reason_code'], default='')
            reason = allw_chrg_node.findtext(xpath_dict['allowance_charge_reason'], default='')
        ...
        return {
            'name': self._retrieve_first_value_in_tree(tree, xpath_dict['name']),
    ...
    def _retrieve_taxes(self, invoice_line, line_values, logs):
        """
    ...
    def _retrieve_line_charges(self, invoice_line, line_values):
        ...
        for charge in line_values.pop('charges'):
            ...
            if charge['reason_code'] == 'AEO':
                tax = self._retrieve_fixed_tax(invoice_line, charge_copy)
                if tax:
                    line_values['tax_ids'] += tax.ids
                    if tax.price_include:
    ...
    def _retrieve_first_value_in_tree(self, tree, xpaths):
        for xpath in xpaths:
            val = self._find_value(xpath, tree)
            if val:
                return val
