...
class AccountEdiXmlUBL20(models.AbstractModel):
    ...
    def _import_fill_invoice(self, invoice, tree, qty_factor):
        ...
        self._import_currency(invoice, tree.find('.//{*}DocumentCurrencyCode'), logs)
        invoice.invoice_date = tree.findtext('./{*}IssueDate')
        invoice.invoice_date_due = self._retrieve_first_value_in_tree(tree, ['./cbc:DueDate', './/cbc:PaymentDueDate'])
        ...
        self._import_prepaid_amount(invoice, tree, './{*}LegalMonetaryTotal/{*}PrepaidAmount', qty_factor, logs)
        line_tag = (
            'InvoiceLine'
            if invoice.move_type in ('in_invoice', 'out_invoice') or qty_factor == -1
            else 'CreditNoteLine'
        )
        self._import_invoice_lines(invoice, tree, './{*}' + line_tag, qty_factor, logs)
        return logs
