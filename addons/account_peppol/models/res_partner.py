...
class ResPartner(models.Model):
    ...
    @api.model
    def _get_access_point_info(self, service_href):
        # If a user is already registered somewhere, we try to show them where exactly
        try:
            response = requests.get(service_href, timeout=TIMEOUT)
            access_point_info = etree.XML(response.content)
            ...
            return access_point_contact or True
        except (requests.exceptions.ConnectionError, etree.XMLSyntaxError):
            return True
    ...
    def _get_participant_info(self, edi_identification):
        ...
        try:
            ...
            return None  # returning None to be consistent with etree
    ...
    def _check_peppol_participant_exists(self, edi_identification, check_company=False, ubl_cii_format=False):
        ...
        if check_company:
            ...
            return self._get_access_point_info(service_href) if service_href else True
    ...
    def _check_document_type_support(self, participant_info, ubl_cii_format):
        ...
        for service in service_metadata.findall('{*}ServiceMetadataReference'):
