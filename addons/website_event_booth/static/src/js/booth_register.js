...
publicWidget.registry.boothRegistration = publicWidget.Widget.extend({
    ...
    _updateErrorDisplay(errors) {
        ...
        let errorMessages = [];
        const errorMessage = this.el.querySelector(".o_wbooth_registration_error_message");
    ...
    _updateUiAfterBoothChange(boothCount) {
        const button = this.el.querySelector("button.o_wbooth_registration_submit");
    ...
    async _onSubmitBoothSelectionClick(ev) {
        ev.preventDefault();
        const form = this.el.querySelector(".o_wbooth_registration_form");
        const event_booth_ids = [...this.el.querySelectorAll("input[name=event_booth_ids]:checked")]
