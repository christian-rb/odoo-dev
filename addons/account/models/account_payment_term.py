...
class AccountPaymentTerm(models.Model):
    ...
    def _compute_interest(self, due_date, payment_date, periodic_interest, lpc_type, company):
        ...
        while current_date <= payment_date:
            ...
            days_late = (period_end - current_date).days + 1
            if days_in_period != 0:
                key = 'Late Pay Charges from {} to {}'.format(current_date.strftime('%Y-%m-%d'), period_end.strftime('%Y-%m-%d'))
                amount = float_round((days_late / days_in_period) * periodic_interest, 2)
                amount_dict[key] = amount
            period_start, period_end = self._get_next_period_dates(period_start, period_end, lpc_type)
            current_date = period_start
        if key in amount_dict:
            new_key = key[:-10] + payment_date.strftime('%Y-%m-%d')
            amount_dict.update(
                {new_key: float_round(amount_dict[key] - (((period_start - relativedelta(days=1)) \
                            - payment_date).days / days_in_period) * periodic_interest, 2)}
            )
        if new_key != key:
            del amount_dict[key]
        return amount_dict
    ...
    def _get_fiscal_dates(self, current_date, lpc_type, company):
        ...
        elif lpc_type in ['quarterly', 'half_yearly']:
    ...
    def _get_next_period_dates(self, period_start, period_end, lpc_type):
        period_start = period_end + relativedelta(days=1)
        if lpc_type == 'monthly':
            months = 1
        elif lpc_type == 'quarterly':
            months = 3
        elif lpc_type == 'half_yearly':
            months = 6
        else:
            months = 12
