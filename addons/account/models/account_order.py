...
class AccountOrderMixin(models.AbstractModel):
    # This class enables downpayment support for models that can have down payments on them
    _name = 'account.order.mixin'
    _description = 'Account Order Mixin'

    amount_to_invoice = fields.Monetary("Amount to invoice", store=True, compute='_compute_amount_to_invoice')
    amount_invoiced = fields.Monetary("Already invoiced", compute='_compute_amount_invoiced')
    ...
    def _sign(self):
        """
        Returns the sign (1 or -1) to multiply with when calculating the amount to invoice and creating the invoice lines.
        """
    ...
    def _filter_subrecords(self):
        """
        Returns a filter function that only returns True for invoice lines linked to this order.
        Used to compute the amount to invoice.
        """
        raise NotImplementedError  # To override
    ...
    def _compute_amount_to_invoice(self):
        for order in self:
            ...
            for invoice in order.invoice_ids.filtered(lambda x: x.state == 'posted'):
                amount_total = sum(invoice.line_ids.filtered(order._filter_subrecords()).mapped('price_total'))
                invoice_amount_currency = invoice.currency_id._convert(
                    amount_total * invoice.direction_sign * order._sign(),
                    order.currency_id,
                    invoice.company_id,
                    invoice.date,
                )
