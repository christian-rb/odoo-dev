...
class AccountBankStatementLine(models.Model):
    ...
    def _compute_journal_id(self):
        for st_line in self:
            st_line.journal_id = st_line.move_id.journal_id

    @api.depends('move_id.company_id')
    def _compute_company_id(self):
        for st_line in self:
            st_line.company_id = st_line.move_id.company_id
    ...
    def _compute_running_balance(self):
        ...
        for journal in self.journal_id:
            ...
            self._cr.execute(SQL(
                ...
                max_index,
                journal.id,
                self.env['res.company'].search([('id', 'child_of', journal.company_id.id)]).ids,
                extra_clause,
            ))
    ...
    def _inverse_company_journal(self):
        for st_line in self:
            vals = {}
            if not st_line.move_id.statement_line_id:
                vals['statement_line_id'] = st_line.id
            if st_line.move_id.journal_id != st_line.journal_id:
                vals['journal_id'] = st_line.journal_id.id
            if st_line.move_id.company_id != st_line.company_id:
                vals['company_id'] = st_line.company_id.id
            if vals:
                st_line.move_id.write(vals)
    ...
    def create(self, vals_list):
        ...
        st_lines = super(AccountBankStatementLine, self.with_context(is_statement_line=True)).create(vals_list)
