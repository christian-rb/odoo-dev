...
class AccountPaymentRegister(models.TransientModel):
    ...
    account_payment_register_dif_ids = fields.One2many('account.payment.register.dif.line', 'account_payment_register_id', store=True)
    ...
    @api.depends('can_edit_wizard', 'account_payment_register_dif_ids.writeoff_account_id')
    def _compute_writeoff_is_exchange_account(self):
        for wizard in self:
            for line in wizard.account_payment_register_dif_ids:
                line.writeoff_is_exchange_account = all((
                    wizard.can_edit_wizard,
                    wizard.currency_id != wizard.source_currency_id,
                    line.writeoff_account_id,
                    line.writeoff_account_id in (
                        line.company_id.expense_currency_exchange_account_id,
                        line.company_id.income_currency_exchange_account_id,
                    ),
                ))
...
class AccountPaymentRegisterDifLine(models.TransientModel):
    ...
    account_payment_register_id = fields.Many2one('account.payment.register', string='Account Payment Register', store=True)
    ...
    writeoff_is_exchange_account = fields.Boolean(default=False, store=True)
