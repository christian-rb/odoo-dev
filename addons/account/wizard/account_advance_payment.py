...
class AdvancePaymentWizard(models.AbstractModel):
    ...
    def _compute_has_down_payments(self):
        for wizard in self:
            wizard.has_down_payments = bool(
                wizard.order_ids.order_line.filtered('is_downpayment')
            )
    ...
    def _compute_display_draft_invoice_warning(self):
        for wizard in self:
            wizard.display_draft_invoice_warning = wizard.order_ids.invoice_ids.filtered(lambda invoice: invoice.state == 'draft')
    ...
    def _create_down_payment_invoice(self):
        ...
        if not any(line.display_type and line.is_downpayment for line in order.order_line):
            self.order_ids.with_context(no_log_for_new_lines=True)._create_new_order_line(
                self._prepare_down_payment_section_values(order)
            )

        values, accounts = self._prepare_down_payment_lines_values(order)
        down_payment_lines = self.order_ids.with_context(no_log_for_new_lines=True)._create_new_order_line(values)
        ...
        if self.advance_payment_method == 'fixed':
            delta_amount = (invoice.amount_total - self.fixed_amount) * (-1 if invoice.is_inbound() else 1) * self.order_ids._sign()
            if not order.currency_id.is_zero(delta_amount):
                receivable_line = invoice.line_ids.filtered(lambda aml: aml.account_id.account_type == self._get_payment_term_account_type())[:1]
    ...
    def _prepare_invoice_values(self, order, dp_lines, accounts):
        self.ensure_one()
        return {
            **order._prepare_account_move(),
