...
class AccountMoveSend(models.TransientModel):
    ...
    def _action_download(self, invoices, attachments):
        """ Download the PDF attachment, or a zip of attachments if there are more than one. """
        return {
            'type': 'ir.actions.act_url',
            'url': f'account/export_invoice_documents/{",".join(map(str, invoices.ids))}'
                   f'?attachment_ids={",".join(map(str, attachments.ids))}',
            'close': True,
        }
    ...
    def _process_send_and_print(self, moves, wizard=None, allow_fallback_pdf=False, **kwargs):
        ...
        if to_download:
            ...
            if attachments:
                ...
                return self._action_download(moves, attachments)
