...
class MailThread(models.AbstractModel):
    ...
    def message_get_followers(self, after=None, limit=20, offset=0, filter_recipients=False):
        ...
        if followers_count <= 150:
            formatted_results = self.env["mail.followers"].search(domain, offset=offset, limit=limit, order="name ASC")._format_for_chatter()
        else:
            if after:
                domain = expression.AND([domain, [('id', '>', after)]])
            formatted_results = self.env["mail.followers"].search(domain, limit=limit, order='id ASC')._format_for_chatter()
