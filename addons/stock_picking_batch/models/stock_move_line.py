...
class StockMoveLine(models.Model):
        ...
        for line in self:
            ...
            if line.picking_type_id.wave_grouping:
                ...
                if line.picking_type_id.wave_group_by_category:
                    if line.product_id.categ_id not in line.picking_type_id.wave_category_ids:
                        continue
    ...
    def _get_possible_waves(self, nearest_parent_location=False):
        ...
        if self.picking_type_id.wave_group_by_category:
            groupby.append('product_categ_id')
        ...
        if self.picking_type_id.wave_group_by_category:
            grouped_lines = [group for group in grouped_lines if group['product_categ_id'][0] == self.product_categ_id.id]
    ...
    def _get_possible_lines_domain(self, nearest_parent_location=False):
        ...
        if self.picking_type_id.wave_group_by_category:
            domain = expression.AND([domain, [('product_categ_id', '=', self.product_categ_id.id)]])
        if self.picking_type_id.wave_group_by_location and nearest_parent_location:
