...
class AccountMove(models.Model):
    ...
    def _check_name(self):
        for move in self:
            if (
                ...
                and move.journal_id.restrict_mode_hash_table
                and move.name
                and move.name != '/'
    ...
    def _get_integrity_hash_fields(self):
        ...
        return ['invoice_date', 'create_date', 'amount_total', 'move_type', 'sequence_prefix', 'sequence_number']
    ...
    def _calculate_hashes(self, previous_hash=None):
        ...
        if not self.journal_id.l10n_pt_at_series_invoice_id and not self.journal_id.l10n_pt_at_series_refund_id:
            raise RedirectWarning(
                message=_(
                    "You must define the official Autoridade Tributária (AT) series for invoices "
                    "and refunds in the journal '%s' to be able to generate the ATCUD."
                ) % self.journal_id.name,
                action={
                    'name': _('Configure the journal'),
        ...
        docs_to_sign = [{
            ...
            'l10n_pt_document_number': move._get_l10n_pt_document_number(),
    ...
    def _compute_l10n_pt_atcud(self):
        for move in self:
            if (
                ...
                at_series = move.journal_id.l10n_pt_at_series_invoice_id if move.move_type == 'out_invoice' else move.journal_id.l10n_pt_at_series_refund_id
                move.l10n_pt_atcud = f"{at_series._get_code()}-{move.sequence_number}"
            else:
