...
class TestMassMailValues(MassSMSCommon):
    ...
    @users('user_marketing')
    def test_mailing_get_replacements_placeholders(self):
        ...
        See `get_replacements_placeholders` for safety characters.
        """
        mailing = self.env['mailing.mailing'].create({
            'name': 'TestMailing',
            'subject': 'Test',
        ...
        self.assertDictEqual(mailing.get_replacements_placeholders(), expected)

        no_mailing = self.env['mailing.mailing']
        self.assertDictEqual(no_mailing.get_replacements_placeholders(), expected)

        new_mailing = self.env['mailing.mailing'].new()
        self.assertDictEqual(new_mailing.get_replacements_placeholders(), expected)
