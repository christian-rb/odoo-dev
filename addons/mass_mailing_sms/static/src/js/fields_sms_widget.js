...
SmsWidget.include({
    ...
    willStart: async function () {
        ...
        if (this.model === "mailing.mailing" && this.mode === 'edit') {
            const { unsubscribe, link } = await this._rpc({
                ...
                method: 'get_replacements_placeholders',
                args: [this.res_id]
            });
            this.replacements_placeholders = { unsubscribe, link };
    ...
    _compute: function() {
        ...
        if (!this.replacements_placeholders) {
            return;
        }
        ...
        if (unsubscribe_el && unsubscribe_el.checked) {
            ...
            this.nbrChar += this.replacements_placeholders.unsubscribe.length;
        }
    },
    /**
    ...
    _getNbrCharExplanationTemplate: function () {
        ...
        return this.noticeLinksReplaced
            ...
            : this._super.apply(this, arguments); // Also default when no replacements_placeholders
    },
    /**
    ...
    _getValueForSmsCounts: function () {
        ...
        if (this.replacements_placeholders) {
            const replaced = value.replaceAll(TEXT_URL_REGEX, this.replacements_placeholders.link);
    ...
    _onClick: function() {
        this._super.apply(this, arguments);
        if (this.replacements_placeholders) {  // covers mailing and edit mode
            this._updateSMSInfo();
        }
    }
});
