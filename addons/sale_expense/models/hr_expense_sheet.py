...
class HrExpenseSheet(models.Model):
    ...
    def _get_sale_order_lines(self):
        """
            ...
            :rtype: recordset
        """
        expensed_amls = self.account_move_id.line_ids.filtered(lambda aml: aml.expense_id and aml.expense_id.sale_order_id and aml.balance >= 0)
        if not expensed_amls:
            return self.env['sale.order.line']
        sale_order_map = expensed_amls._sale_determine_order()
        sale_order_ids = tuple(set(sale_order_map[aml.id].id for aml in expensed_amls))
        aml_sol_price_map = dict(expensed_amls.mapped(lambda aml: (aml.id, aml._sale_get_invoice_price(sale_order_map[aml.id]))))
        product_ids = tuple(expensed_amls.mapped('product_id').ids)
        ...
        """
        self.env.cr.execute(query, (sale_order_ids, product_ids, quantities, tuple(set(aml_sol_price_map.values())), names))
        sol_ids_raw = {
        ...
        expensed_amls_keys = set(expensed_amls.mapped(
            lambda aml: (aml.expense_id.sale_order_id.id, aml.product_id.id, aml.quantity, aml_sol_price_map[aml.id], aml.name)
        ))
        return self.env['sale.order.line'].browse(sol_id for key, sol_id in sol_ids_raw.items() if key in expensed_amls_keys)

    def action_reset_expense_sheets(self):
        sale_order_lines = self._get_sale_order_lines()
        if sale_order_lines:
            sale_order_lines.write({'qty_delivered': 0., 'product_uom_qty': 0.})
