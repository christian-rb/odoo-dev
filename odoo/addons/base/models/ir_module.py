...
def assert_log_admin_access(method):
    ...
    def check_and_log(method, self, *args, **kwargs):
        ...
        if not self.env.is_admin():
            _logger.warning('%r DENY access to module.%s on %s to user %s (#%d) via %s',
                            LogType.IR_MODULE_ACCESS_DENIED, *log_data)
            raise AccessDenied()
        _logger.info('%r ALLOW access to module.%s on %s to user %s (#%d) via %s', LogType.IR_MODULE_ACCESS_ALLOWED,
                    *log_data)
