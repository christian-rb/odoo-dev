...
class Company(models.Model):
    ...
    def install_l10n_modules(self):
        companies = self.filtered('uninstalled_l10n_module_ids')
        is_ready_and_not_test = (
            not tools.config['test_enable']
            and (self.env.registry.ready or not self.env.registry._init)
        )
        if companies and is_ready_and_not_test:
            return companies.uninstalled_l10n_module_ids.button_immediate_install()
    ...
    def create(self, vals_list):
        ...
        companies.install_l10n_modules()
